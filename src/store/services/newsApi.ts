...
import { CategoriesApiResponse, NewApiResponse, ParamsType } from '../../interfaces';
...
export const newsApi = createApi({
    ...
    endpoints: (builder) => ({
        getNews: builder.query<NewApiResponse, ParamsType>({
            keepUnusedDataFor: 0,
            query: (params) => {
                const {
                    page_number = 1,
        ...
        getLatestNews: builder.query<NewApiResponse, null>({
            query: () => {                
                return {
                    url: 'latest-news',
                    params: {
